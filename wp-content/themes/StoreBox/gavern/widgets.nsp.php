...
class GK_NSP_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		} else if($config['data_source_type'] == 'wooc_latest') {
			$results = get_posts(array(
				'posts_per_page' => $amount_of_posts,
				'post_type' => 'product',
				'offset' => $config['offset'], 
		...
		<div class="gk-nsp-col">
			...
			<p>
				...
				<select id="<?php echo esc_attr( $this->get_field_id('data_source_type')); ?>" name="<?php echo esc_attr( $this->get_field_name('data_source_type')); ?>">
					...
					<option value="wooc_latest"<?php selected($data_source_type, 'wooc_latest'); ?>>
						<?php _e('Latest WooCommerce products', GKTPLNAME); ?>
					</option>
					<option value="wooc_category"<?php selected($data_source_type, 'wooc_category'); ?>>
						<?php _e('WooCommerce Categories slugs', GKTPLNAME); ?>
					</option>
					<option value="wooc_post"<?php selected($data_source_type, 'wooc_post'); ?>>
	 ...
	 **/
	 ...
	 function generate_art_info($i) {
	 	...
	 	if (!function_exists('get_product')) {
	 		return false;
	 	} elseif($this->wdgt_config['data_source_type'] == 'wooc_latest' || $this->wdgt_config['data_source_type'] == 'wooc_category' || $this->wdgt_config['data_source_type'] == 'wooc_post') {
	 		$prod = get_product($art_ID); 
	 	}
	 	// check if there is a category in format
	 	if($this->wdgt_config['data_source_type'] == 'wooc_latest' || $this->wdgt_config['data_source_type'] == 'wooc_category' || $this->wdgt_config['data_source_type'] == 'wooc_post') {
	 		if(stripos($this->wdgt_config['article_info_format'], '%CATEGORY') !== FALSE && $prod) {				
	 	...
	 	if(stripos($this->wdgt_config['article_info_format'], '%PRICE') !== FALSE && $prod) {	 		
	 		// generate the output
	 		$price = $prod->get_price_html();
	 	}
	 	// replace them all!
	 	$output = str_replace(
	 		array('%CATEGORY', '%AUTHOR', '%DATE', '%COMMENTS', '%PRICE'),
	 		array($category, $author, $date, $comments, $price),
